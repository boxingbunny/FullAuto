...
public class AdminPanel
{
    ...
    {
        // 不能踢自己
        if (user.Id == RoomClientManager.Instance.Client.PlayerId)
        {
            ImGui.TextDisabled("-");
            return;
        }
    ...
    private async Task RefreshUserListAsync()
    {
        ...
        try
        {
            var ack = await RoomClientManager.Instance.Client.GetAllUsersAsync();
            if (ack?.Success == true)
            {
                // 等待一帧让消息处理完成
                await Task.Delay(100);
                // 从状态获取用户列表（由 OnWebSocketMessage 更新）
                _userList = RoomClientState.Instance.AllConnectedUsers;
        ...
        {
            LogHelper.Error($"[RoomClient] 刷新用户列表异常: {ex}");
    ...
    private async Task KickUserAsync(string userId, string userName)
    {
        ...
        var ack = await RoomClientManager.Instance.Client.KickUserAsync(userId);
        if (ack?.Success == true)
        {
