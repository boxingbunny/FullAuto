...
namespace AutoRaidHelper.UI
{
    ...
    public class AutomationTab
    {
        ...
        // 判断是否在新月岛CE内
        private static unsafe bool IsInsideCriticalEncounter(Vector3 pos, bool includeRegisterWarmup = false, float radius = 20f)
        {
            ...
            foreach (ref var events in instance->DynamicEventContainer.Events)
            {
                if (events.DynamicEventId == 0)
                    continue;
                if (!(events.State == DynamicEventState.Battle || (includeRegisterWarmup && events.State is DynamicEventState.Register or DynamicEventState.Warmup)))
                    continue;
