...
public class RoomManagementPanel
{
    ...
    public RoomManagementPanel(FullAutoPlugin plugin)
    {
        _plugin = plugin;
        _ownerControls = new RoomOwnerControls(plugin);
    }
    ...
    private void DrawLobbyView()
    {
        ...
        if (ImGui.CollapsingHeader("创建房间", ImGuiTreeNodeFlags.DefaultOpen))
        {
            ...
            if (ImGui.Button("创建房间##Create"))
            {
                ...
                {
                    FullAutoState.Instance.StatusMessage = "请输入房间名称";
                }
                else
                {
        ...
        if (ImGui.CollapsingHeader("房间列表", ImGuiTreeNodeFlags.DefaultOpen))
        {
            ...
            ImGui.SameLine();
            ImGui.Text($"共 {FullAutoState.Instance.RoomListTotal} 个房间");

            ImGui.BeginChild("RoomList", new Vector2(0, 250), true);

            foreach (var room in FullAutoState.Instance.RoomList)
            {
            ...
            {
                var selectedRoom = FullAutoState.Instance.RoomList.Find(r => r.Id == _selectedRoomId);
    ...
    {
        var state = FullAutoState.Instance;
        ...
        if (ImGui.Button("刷新"))
        {
            _ = _plugin.Client.GetRoomInfoAsync(room.Id);
        }
    ...
    /// 绘制简单玩家列表（8人及以下房间）
    /// </summary>
    private void DrawSimplePlayerList(FullAutoState state, RoomInfo room)
    {
        ImGui.Text("玩家列表:");
    ...
    /// 绘制分组玩家列表（大于8人房间）
    /// </summary>
    private void DrawGroupedPlayerList(FullAutoState state, RoomInfo room)
    {
        // 按小队分组玩家
    ...
    private async Task CreateRoomSafeAsync()
    {
        ...
        {
            AEAssist.Helper.LogHelper.Error($"[FullAuto] 创建房间异常: {ex}");
            FullAutoState.Instance.StatusMessage = $"创建房间异常: {ex.Message}";
        }
    }
    ...
    private async Task RefreshRoomListSafeAsync()
    {
        try
        {
            await _plugin.Client.GetRoomListAsync();
        }
        catch (OperationCanceledException)
        {
            // 操作被取消，不是错误
        ...
        {
            AEAssist.Helper.LogHelper.Error($"[FullAuto] 刷新房间列表异常: {ex}");
            FullAutoState.Instance.StatusMessage = $"刷新房间列表异常: {ex.Message}";
        }
    }
    ...
    {
        FullAutoState.Instance.StatusMessage = "正在创建房间...";
        var size = (RoomSize)RoomSizes[_createRoomSizeIndex];
        var ack = await _plugin.Client.CreateRoomAsync(_createRoomName, size, _createRoomPassword);

        if (ack?.Success == true)
        {
            FullAutoState.Instance.StatusMessage = "房间创建成功";
            _createRoomName = "";
            _createRoomPassword = "";
        }
        else
        {
            FullAutoState.Instance.StatusMessage = $"创建失败: {ack?.Error ?? "未知错误"}";
        }
    }

    private async Task JoinRoomAsync(string roomId, string password)
    {
        var ack = await _plugin.Client.JoinRoomAsync(roomId, password);

        if (ack?.Success == true)
        {
            FullAutoState.Instance.CurrentRoomId = roomId;
            FullAutoState.Instance.StatusMessage = "加入房间成功";
            _joinRoomPassword = "";
            _selectedRoomId = "";

            await _plugin.Client.GetRoomInfoAsync(roomId);
        }
        else
        {
            FullAutoState.Instance.StatusMessage = $"加入失败: {ack?.Error ?? "未知错误"}";
        }
    }

    private async Task LeaveRoomAsync()
    {
        var ack = await _plugin.Client.LeaveRoomAsync();

        if (ack?.Success == true)
        {
            FullAutoState.Instance.ClearRoomState();
            FullAutoState.Instance.StatusMessage = "已离开房间";
        }
        else
        {
            FullAutoState.Instance.StatusMessage = $"离开失败: {ack?.Error ?? "未知错误"}";
        }
    }

    private async Task DisbandRoomAsync()
    {
        var roomId = FullAutoState.Instance.CurrentRoomId;
        if (string.IsNullOrEmpty(roomId)) return;

        var ack = await _plugin.Client.DisbandRoomAsync(roomId);

        if (ack?.Success == true)
        {
            FullAutoState.Instance.ClearRoomState();
            FullAutoState.Instance.StatusMessage = "房间已解散";
        }
        else
        {
            FullAutoState.Instance.StatusMessage = $"解散失败: {ack?.Error ?? "未知错误"}";
        }
    }
