...
public class RoomOwnerControls
{
    ...
    public RoomOwnerControls(FullAutoPlugin plugin)
    {
        _plugin = plugin;
    }
    ...
    private void DrawRoleCombo(RoomPlayer player, RoomInfo room)
    {
        ...
        // 获取该队伍内已被占用的职能
        var occupiedRoles = FullAutoState.Instance.GetOccupiedRolesInTeam(player.TeamId, player.Id);
        ...
        if (ImGui.Combo("##role", ref roleIndex, displayLabels, displayLabels.Length))
        {
            ...
            {
                FullAutoState.Instance.StatusMessage = $"职能 {newRole} 已被占用";
            }
            else
            {
    ...
    {
        // 不能踢自己
        if (player.Id == _plugin.Client.PlayerId)
        {
            ImGui.TextDisabled("-");
            return;
        }
    ...
    {
        var ack = await _plugin.Client.AssignTeamAsync(roomId, playerId, teamId);
        if (ack?.Success == true)
        {
            // 刷新房间信息以更新显示
        ...
        {
            FullAutoState.Instance.StatusMessage = $"分配队伍失败: {ack?.Error ?? "未知错误"}";
        }
    }
    ...
    private async Task AssignRoleSafeAsync(string roomId, string playerId, string role)
    {
        ...
        {
            AEAssist.Helper.LogHelper.Error($"[FullAuto] 分配职能异常: {ex}");
            FullAutoState.Instance.StatusMessage = $"分配职能异常: {ex.Message}";
        }
    }

    private async Task AssignRoleAsync(string roomId, string playerId, string role)
    {
        var ack = await _plugin.Client.AssignRoleAsync(roomId, playerId, role);
        if (ack?.Success == true)
        {
            // 刷新房间信息以更新显示
            await _plugin.Client.GetRoomInfoAsync(roomId);
        ...
        {
            FullAutoState.Instance.StatusMessage = $"分配职能失败: {ack?.Error ?? "未知错误"}";
        }
    }

    private async Task KickPlayerAsync(string roomId, string playerId)
    {
        var ack = await _plugin.Client.KickPlayerAsync(roomId, playerId);
        if (ack?.Success == true)
        {
            FullAutoState.Instance.StatusMessage = "玩家已被踢出";
            await _plugin.Client.GetRoomInfoAsync(roomId);
        }
        else
        {
            FullAutoState.Instance.StatusMessage = $"踢出失败: {ack?.Error ?? "未知错误"}";
        }
    }
